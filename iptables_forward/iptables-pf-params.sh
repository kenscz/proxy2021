...
Add_iptables(){
	iptables -t nat -A PREROUTING -4 -p "$1" --dport "${local_port}" -j DNAT --to-destination "${forwarding_ip}":"${forwarding_port}"
	iptables -t nat -A POSTROUTING -4 -p "$1" -d "${forwarding_ip}" --dport "${forwarding_port_1}" -j MASQUERADE
